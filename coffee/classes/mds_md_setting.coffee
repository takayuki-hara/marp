...
module.exports = class MdsMdSetting
  ...
  @transformers:
    ...
    theme: (v) ->
      ...
      return if basename in ['default', 'gaia', 'haskell', 'olive', 'colors', 'colors-hannari', 'colors-kokoro', 'colors-mplus1p', 'colors-nicomoji'] then "css/themes/#{basename}.css" else null
    template: (v) -> v
    footer: (v) -> v
